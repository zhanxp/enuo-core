...
function MongoDB() {
        ...
        return new Promise(function(resolve, reject) {
            MongoClient.connect(dbUrl, options, function(err, db) {
                ...
                if (!db_name) {
                    ...
                    database.command({ connectionStatus: 1, showPrivileges: true }).then(function(status) {
                        if (status.ok == 1) {
                            _this.db = database;
                            logger.info("已经切换到 mongodb db_name：", db_name);
    ...
    this.updateById = async function(collectionName, doc, _id, options) {
        return await this.updateOne(collectionName, doc, { _id: _id }, options);
    }
